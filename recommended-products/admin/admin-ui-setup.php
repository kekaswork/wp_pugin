...
function prefix_add_menu_links() {
	add_options_page ( __('Recommended Products','recommended-products'), __('Recommended Products','recommended-products'), 'update_core', 'recommended-products','prefix_admin_interface_render'  );
}
...
function prefix_register_settings() {
    ...
    add_settings_section(
        ...
        __('Recommended Products General Settings', 'recommended-products'),		// Title
        'prefix_general_settings_section_callback',					// Callback Function
        'recommended-products'											// Page slug
    );
	
	// General Settings
    add_settings_field(
        'prefix_general_settings_field',							// ID
        __('General Settings', 'recommended-products'),					// Title
...
function prefix_enqueue_css_js( $hook ) {
	...
	if ( $hook != "settings_page_recommended-products" ) {
		return;
	}
	
	// Main CSS
	// wp_enqueue_style( 'prefix-admin-main-css', PREFIX_RECOMMENDED_PRODUCTS_URL . 'admin/css/main.css', '', PREFIX_VERSION_NUM );
	
	// Main JS
    // wp_enqueue_script( 'prefix-admin-main-js', PREFIX_RECOMMENDED_PRODUCTS_URL . 'admin/js/main.js', array( 'jquery' ), false, true );
}
